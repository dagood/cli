...
if [ $BUILD -eq 1 ]; then
    (set -x; dotnet msbuild build.proj '/flp:v=diag;logfile=msbuild.writedynamicprops.txt' /p:Architecture=$ARCHITECTURE $CUSTOM_BUILD_ARGS /p:GeneratePropsFile=true /t:WriteDynamicPropsToStaticPropsFiles $argsnotargets)
    (set -x; dotnet msbuild build.proj /m /v:normal /fl /flp:v=diag /p:Architecture=$ARCHITECTURE $CUSTOM_BUILD_ARGS $args)
else
